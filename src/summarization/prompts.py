...
def few_shot_comparison_prompt(products: list, category: str, sample_reviews: dict = None) -> str:
    ...
    product_sections = []
    for p in products:
        product_section = f"- {p['name']}: {p['avg_rating']}/5 rating, {p['review_count']} reviews, {p['positive_ratio']*100:.0f}% positive"
        ...
        if sample_reviews and p['name'] in sample_reviews:
            ...
            critical_reviews = [r for r in reviews if r['sentiment'] in ['negative', 'neutral']][:2]
            
            if positive_reviews or critical_reviews:
                product_section += "\n  Customer Feedback:"
                
                for review in positive_reviews:
                    product_section += f"\n    ✓ \"{review['text'][:150]}{'...' if len(review['text']) > 150 else ''}\" ({review['rating']}/5)"
    ...
    return f"""Create a product recommendation guide for {category} based on customer reviews and ratings.
